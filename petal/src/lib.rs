...
pub mod sdk {
    pub use super::{
        ...
        OutboxInspection, SdkError, StagedTransaction,
    };
    ...
    fn namespace_for_key(key: &str, secret: bool) -> &'static str {
        if secret || key.starts_with("creds/") || key.starts_with("credentials/") {
            SECRET_NS
        } else {
            STATE_NS
        }
...
impl RouteSpec {
    ...
    pub const fn async_write(mut self) -> Self {
        self.write_async = true;
        self
    }
}
...
pub fn write_spec() -> RouteSpec {
    RouteSpec::writable().caps(CAPS_WORKFLOW).ttl(None)
}

pub fn async_write_spec() -> RouteSpec {
    write_spec().async_write()
}
